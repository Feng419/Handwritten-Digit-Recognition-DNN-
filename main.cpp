...
float train(Network *network, string path, int imageSize,
            int numImages) { //训练网络，使用训练数据集
    srand(time(0));
    ...
    for (int i = 0; i < numImages; i++) {
        ...
        string l = la;
        Mat x = imread(l.append(to_string(k)).append(".png"), 0); //使用 opencv 读取图片
...
int validate(Network *network, string path, int imageSize,
             int numImages) { //验证网络准确率，使用测试数据集
    int ok_cnt = 0;
    float *temp = new float[imageSize];
    string la = path;
    ...
    for (int i = 0; i < numImages; i++) {
        ...
        string l = la;
        Mat x = imread(l.append(to_string(i)).append(".png"), 0); //按顺序读取图片测试
...
int main(int argc, char *argv[]) { //主入口函数
    if (argc < 2 || (strcmp(argv[1], "train") != 0 &&
                     strcmp(argv[1], "test") != 0)) { //判断调用参数是否合法
    ...
    if (strcmp(argv[1], "train") == 0) { //训练网络
        ...
        cout << "total epoches: " << network->mEpoches << ", NO.1 epoches. begin learning rate: " <<
             network->mLearningRate << endl;
        ...
        for (int i = 0; i < network->mEpoches; i++) { //共训练 epoches 轮次
            ...
            cout << "validate accuracy: " << (float)ok / testNumImages * 100 << "%, true: " << ok << ", total: " <<
                 testNumImages << endl;

            if (network->mLearningRate > 0.01)
                network->mLearningRate -= 0.01; //学习率变化调整
            else
                network->mLearningRate = 0.01;

            if (i < network->mEpoches - 1)
                saveWeight(weightFile.append(to_string(i + 1)).append("_").append(to_string(ok)),
                           network); //一轮训练结束，保存权重文件
        }
        ...
        while (1) { //循环测试图片
            ...
            if (m.cols != imageCol || m.rows != imageRow)
                resize(m, m, Size(imageCol, imageRow)); //resize 图片到网络接受输入大小
